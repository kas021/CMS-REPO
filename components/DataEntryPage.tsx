...
const DataEntryPage: React.FC<DataEntryPageProps> = ({ jobs, drivers, customers, invoices, onSaveJob, onDeleteJob, onDeleteJobs, onUpdateStatus, onAssignDriver, tabId }) => {
    ...
    const handleSaveJob = useCallback((jobData: JobFormData) => {
        onSaveJob(jobData);
        if (!jobData.id) { // This is a new job
            setActiveTab(JobStatus.PENDING);
        }
        setSelectedJob(null);
    ...
    return (
        <div className="space-y-6 p-6">
            <h1 className="text-2xl font-bold text-brand-blue-dark">Data Entry</h1>
            <div className="p-6 bg-white rounded-lg shadow-md">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 items-end">
                    <div className="lg:col-span-2 xl:col-span-1">
                        <label htmlFor="search-text" className="text-sm font-medium text-gray-600">Company/AWB/Ref</label>
                        <input id="search-text" type="text" value={searchText} onChange={(e) => setSearchText(e.target.value)} className="mt-1 w-full p-2 border rounded-md focus:ring-brand-blue focus:border-brand-blue" />
                    </div>
                    <div>
                        <label className="text-sm font-medium text-gray-600">Date From</label>
                        <input type="date" className="mt-1 w-full p-2 border rounded-md focus:ring-brand-blue focus:border-brand-blue" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select className="mt-1 w-full p-2 border rounded-md focus:ring-brand-blue focus:border-brand-blue">
